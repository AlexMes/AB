    ...
    <div class="flex flex-col items-center relative" >
        <div class="w-full">
            <div class="my-2 bg-white p-1 flex border border-gray-200 rounded">
                ...
                <div>
                    <button x-on:click.prevent="isOpen('{{ $id }}') ? search.{{ $id }}='' : removeAll('{{ $id }}')" class="cursor-pointer w-6 h-full flex items-center text-gray-400 outline-none focus:outline-none">
                ...
                <div class="text-gray-300 w-8 py-1 pl-2 pr-1 border-l flex items-center border-gray-200">
                    <button x-show="!isOpen('{{ $id }}')" x-on:click.prevent="open('{{ $id }}')" class="cursor-pointer w-6 h-6 text-gray-600 outline-none focus:outline-none">
                    ...
                    <button x-show="isOpen('{{ $id }}')" x-on:click.prevent="close('{{ $id }}')" class="cursor-pointer w-6 h-6 text-gray-600 outline-none focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-chevron-up w-4 h-4">
            ...
            <div class="flex flex-col w-full">
                <template x-for="(option,index) in options.{{ $id }}" :key="option">
                    <div x-show="!option.filtered">
                        <div @click="select('{{ $id }}',index,$event)" class="cursor-pointer w-full border-gray-100 rounded-t border-b hover:bg-teal-100" style="">
                            <div x-bind:class="option.selected ? 'border-teal-600' : ''" class="flex w-full items-center p-2 pl-2 border-transparent bg-white border-l-2 relative hover:bg-teal-600 hover:text-teal-100 hover:border-teal-600">
...
<select id="{{ $id }}"
    ...
    @foreach($options as $option)
        @php $value = is_array($options) ? $option[$trackBy] : $option->$trackBy @endphp
        @php $text = is_array($options) ? $option[$label] : $option->$label @endphp
        <option
            value="{{$value}}" {{ in_array($value, Arr::wrap(request($id, []))) ? 'selected' : '' }}>{{ $text }}</option>
    @endforeach
</select>

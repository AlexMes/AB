...
class MigrateToTeslaa extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'fixture:migrate-tesla-to-teslaa';
     ...
     * @var string
     */
    protected $description = 'Migrate affiliate leads to offer TESLAA';
    ...
    {
        // Migrate affiliates
        $this->task('Migrate affiliates', fn () => Affiliate::whereOfferId(3)->update(['offer_id' => 56]));
        // Migrate leads
        $this->task(
            'Migrate leads',
            fn () =>
            Lead::where('offer_id', 54)
        ...
        $this->task('Migrate routes', function () {
            $routes = LeadOrderRoute::whereOfferId(54)
                ->whereHas('order', function (Builder $builder) {
                    $builder->whereDate('date', '>', '2020-08-30');
                })->cursor();
        ...
        $this->task('Update results', function () {
            ...
            $orders = LeadsOrder::whereDate('date', '>', '2020-08-30')->cursor();
    ...
    protected function splitRoutesByAssignmentOffer($route)
    {
        ...
        $foreignAssignments = $route->assignments()->whereHas(
            'lead',
            fn ($query) => $query->where('offer_id', 56)
        )->get();

...
class PullOptimusResults extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'postback:collect-optimus-leads-results';
     ...
     * @var string
     */
    protected $description = 'Pull results from Optimus integration';
    ...
    {
        $destinations = LeadDestination::whereDriver(LeadDestinationDriver::OPTIMUS)->get();
        ...
        foreach ($destinations as $destination) {
            if (LeadOrderAssignment::where('destination_id', $destination->id)->exists()) {
                ...
                /** @var \App\DestinationDrivers\Optimus $handler */
                ...
                collect($ftd)->each(function ($deposit) use ($destination) {
                    $assignment = LeadOrderAssignment::query()
                        ->with('lead')
                        ->whereExternalId($deposit['user_id'])
                        ->where('destination_id', $destination->id)
                        ->first();

...
@section('content')
    <div x-data="AppsIndex()" x-init="init()" class="mt-8">
        <div class="flex flex-col">
            ...
            @endif
            <form action="{{ route('deluge.apps.index') }}" class="flex flex-col items-center mb-6 md:flex-row">
                <div class="w-1/6 mr-4 md:w-1/5 sm:col-span-3">
                    ...
                    <div class="mt-1 rounded-md shadow-sm">
                        <select id="status"
                            ...
                            <option value="" @if(request('status') === null) selected @endif>Все</option>
                            <option value="new" @if(request('status') == 'new') selected @endif>Новое</option>
                            <option value="published" @if(request('status') == 'published') selected @endif>Опубликовано</option>
                            <option value="banned" @if(request('status') == 'banned') selected @endif>Забанено</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mx-4 mt-5 sm:col-span-3">
                    <span class="inline-flex rounded-md shadow-sm">
                      <a href="{{ route('deluge.apps.create') }}"
                         class="inline-flex items-center px-3 py-2 -mb-px text-sm font-medium leading-4 text-white transition duration-150 ease-in-out bg-indigo-600 border border-transparent rounded-md hover:bg-indigo-500 focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-indigo-700">
            ...
            @include('deluge::apps.index-data')
        </div>
    </div>
@endsection

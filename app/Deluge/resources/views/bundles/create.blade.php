...
@section('content')
    ...
    <div x-data="BundlesForm()" x-init="init()" class="bg-white shadow overflow-visible sm:rounded-md mt-8 max-w-7xl mx-auto">
        ...
        <form method="post" action="{{ route('deluge.bundles.store') }}" class="px-6">
            @csrf
            <div
            ...
            <div
                class="mt-6 sm:mt-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                <label for="traffic_source_id" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Источник
                </label>
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <select id="traffic_source_id"
                                name="traffic_source_id"
                                class="form-select block w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                            <option value="" {{ old('traffic_source_id') === null ? 'selected' : '' }}>Не выбрана</option>
                            @foreach($trafficSources as $trafficSource)
                                <option
                                    value="{{ $trafficSource->id }}" {{ old('traffic_source_id') == $trafficSource->id ? 'selected' : '' }}>{{ $trafficSource->name }}</option>
                            @endforeach
                        </select>
                    </div>

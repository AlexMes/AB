...
@section('content')
    ...
    <div x-data="CampaignsForm()" x-init="init()" class="bg-white shadow overflow-visible sm:rounded-md mt-8 max-w-7xl mx-auto">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">
                Новая кампания
            </h3>
        </div>
        <form method="post" action="{{ $action ?? route('deluge.campaigns.store') }}" class="px-6">
            @csrf
            <div
            ...
            <div
                ...
                <div class="@isset($account_id)mt-1 sm:mt-0 @else -my-2 @endisset sm:col-span-2">
                    @isset($account_id)
                        {{ $account_id }}
                    ...
                    @else
                        <div class="max-w-xs">
                            @component('deluge::components.multiselect', [
                                    ...
                                    'label' => 'name',
                                    'multiple' => false,
                                    'selected' => old('account_id', $account_id ?? null),
                                ]) @endcomponent
                        </div>
                    @endisset
            ...
            <div
                class="mt-6 sm:mt-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                <label for="creo" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Креатив
                </label>
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
            ...
            <div
                class="mt-6 sm:mt-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                <label for="bundle_id" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Связка
                </label>
                <div class="-my-2 sm:col-span-2">
                    <div class="max-w-xs">
                        @component('deluge::components.multiselect', [
                                ...
                                'label' => 'name',
                                'multiple' => false,
                                'selected' => old('bundle_id'),
                            ]) @endcomponent
                    </div>
                    @error('bundle_id') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>
            <div class="mt-6 border-t border-gray-200 py-5">

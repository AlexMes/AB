...
@section('content')
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md mt-8 max-w-7xl mx-auto">
        ...
        <form method="post" action="{{ route('deluge.credit-cards.store') }}" class="px-6">
            @csrf
            <div
                    ...
                    <div class="max-w-xs rounded-md shadow-sm">
                        <select id="buyer_id"
                            ...
                            @foreach($buyers as $buyer)
                                <option
                                    value="{{ $buyer->id }}" {{ old('buyer_id') == $buyer->id ? 'selected' : '' }}>{{ $buyer->name }}</option>
                            @endforeach
                        </select>
                    </div>

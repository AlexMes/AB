@extends('deluge::app')
@section('title', 'Редактирование карты')
@section('content')
    @if($errors->any())
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md mt-8 max-w-7xl mx-auto">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">
                {{ sprintf('[#%d]',$creditCard->id) }}
            </h3>
        </div>
        <form method="post" action="{{ route('deluge.credit-cards.update', $creditCard) }}" class="px-6">
            @method('PUT')
            @csrf
            <div
                class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-gray-200 sm:pt-5">
                <label for="bank_name" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Банк
                </label>
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <input id="bank_name"
                               name="bank_name"
                               value="{{ old('bank_name', $creditCard->bank_name) }}"
            ...
            <div
                ...
                <label for="number" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Карта
                </label>
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
            ...
            <div
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <input id="account_id"
                               name="account_id"
                               value="{{ old('account_id', $creditCard->account_id) }}"
                               class="form-input block w-full sm:text-sm sm:leading-5"
                               type="text"/>
                    </div>
            ...
            <div
                class="mt-6 sm:mt-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                <label for="buyer_id" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Байер
                </label>
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <select id="buyer_id"
                            ...
                            <option value="" {{ old('buyer_id') === null ? 'selected' : '' }}>Не выбран</option>
                            @foreach($buyers as $buyer)
                                <option
                                    value="{{ $buyer->id }}" {{ old('buyer_id', $creditCard->buyer_id) == $buyer->id ? 'selected' : '' }}>{{ $buyer->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('buyer_id') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>

            <div
                class="mt-6 sm:mt-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                <label for="social_profile" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Соц. профиль
                </label>
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <input id="social_profile"
                               name="social_profile"
                               value="{{ old('social_profile', $creditCard->social_profile) }}"
                               class="form-input block w-full sm:text-sm sm:leading-5"
                               type="text"/>
                    </div>
                    @error('social_profile') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>

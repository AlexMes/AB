...
@section('content')
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md mt-8 max-w-7xl mx-auto">
        ...
        <form method="post" action="{{ route('deluge.groups.update', $group) }}" class="px-6">
            @method('PUT')
            @csrf
            <div
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <select id="branch_id"
                            ...
                            <option
                                value="" {{ old('branch_id', $group->branch_id) === null ? 'selected' : '' }}>Не выбран
                            </option>
                            @foreach($branches as $branch)
                                <option
                                    value="{{ $branch->id }}" {{ old('branch_id', $group->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                    </div>

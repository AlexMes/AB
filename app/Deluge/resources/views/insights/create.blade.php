...
@section('content')
    ...
    <div x-data="InsightsForm()" x-init="init()" class="bg-white shadow overflow-hidden sm:rounded-md mt-8 max-w-7xl mx-auto">
        ...
        <form method="post" action="{{ $action ?? route('deluge.insights.store') }}" class="px-6">
            @csrf
            <div
                    ...
                    @else
                        <div class="max-w-xs rounded-md shadow-sm">
                            <select id="campaign_id"
                                ...
                                <option value="" {{ old('campaign_id') === null ? 'selected' : '' }}>Не выбрана</option>
                                @foreach($campaigns as $camp)
                                    <option
                                        value="{{ $camp->id }}" {{ old('campaign_id', $campaign_id ?? null) == $camp->id ? 'selected' : '' }}>{{ $camp->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endisset

    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md mt-8 max-w-7xl mx-auto">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">
                {{ sprintf('[#%d] %s ',$supplier->id, $supplier->name) }}
            </h3>
        </div>
        <form method="post" action="{{ route('deluge.suppliers.update', $supplier) }}" class="px-6">
            @method('PUT')
            @csrf
            <div
            ...
            <div
                ...
                <label for="branch_id" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Филиал
                </label>
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <select id="branch_id"
                            ...
                            <option
                                value="" {{ old('branch_id', $supplier->branch_id) === null ? 'selected' : '' }}>Не выбран
                            </option>
                            @foreach($branches as $branch)
                                <option
                                    value="{{ $branch->id }}" {{ old('branch_id', $supplier->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div
                class="mt-6 sm:mt-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                <label for="google" class="block text-sm font-medium leading-5 text-gray-700 sm:mt-px sm:pt-2">
                    Google
                </label>
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <input id="google"
                               name="google"
                               value="{{ old('google', $supplier->google) }}"
                               class="form-input block w-full sm:text-sm sm:leading-5"
                               type="text"/>
                    </div>
                    @error('google') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>

...
@section('content')
    ...
    <div class="bg-white shadow overflow-visible sm:rounded-md mt-8 max-w-7xl mx-auto">
        <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">
                {{ sprintf('[#%d] %s ',$trafficSource->id, $trafficSource->name) }}
            </h3>
        </div>
        <form method="post" action="{{ route('deluge.traffic-sources.update', $trafficSource) }}" class="px-6">
            @method('PUT')
            @csrf
            <div
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <input id="name"
                               name="name"
                               value="{{ old('name', $trafficSource->name) }}"

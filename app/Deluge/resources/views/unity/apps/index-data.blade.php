                    ...
                    <tbody class="bg-white">
                        @foreach($unityApps as $unityApp)
                            <tr class="cursor-pointer hover:bg-gray-100"
                                onclick="window.location = '{{ route('deluge.unity.apps.show', $unityApp->id) }}'">
                                <td class="px-5 py-5 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">
                                    <div class="font-semibold">{{ $unityApp->name }}</div>
                                    <div class="text-xs text-gray-600">{{ $unityApp->id }}</div>
                                </td>
                                <td class="px-5 py-5 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">
                ...
                <nav class="flex items-center justify-between px-4 py-3 bg-white border-gray-200 sm:px-6">
                    <div class="hidden sm:block">
                        <p class="text-sm leading-5 text-gray-700">
                            От
                            <span class="font-medium">{{ $unityApps->firstItem() }}</span>
                            до
                            <span class="font-medium">{{ $unityApps->lastItem() }}</span>
                            из
                            <span class="font-medium">{{ $unityApps->total() }}</span>
                            приложений
                        </p>
                    </div>
                    <div class="flex justify-between flex-1 sm:justify-end">
                        @if($unityApps->previousPageUrl())
                            <a href="{{ $unityApps->appends(request(['organization']))->previousPageUrl() }}"
                        ...
                        @endif
                        @if($unityApps->nextPageUrl())
                            <a href="{{ $unityApps->appends(request(['organization']))->nextPageUrl() }}"

...
@section('content')
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md mt-8 max-w-7xl mx-auto">
        ...
        <form method="post" action="{{ route('deluge.unity.insights.update', $insight) }}" class="px-6">
            @method('PUT')
            @csrf
            <div
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <select id="app_id"
                            ...
                            <option
                                value="" {{ old('app_id', $insight->app_id) === null ? 'selected' : '' }}>Не выбран
                            </option>
                            @foreach($unityApps as $unityApp)
                                <option
                                    value="{{ $unityApp->id }}" {{ old('app_id', $insight->app_id) == $unityApp->id ? 'selected' : '' }}>{{ $unityApp->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <select id="campaign_id"
                                ...
                                class="form-select block w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                            <option
                                value="" {{ old('campaign_id', $insight->campaign_id) === null ? 'selected' : '' }}>Не выбран
                            </option>
            ...
            <div
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-xs rounded-md shadow-sm">
                        <input id="installs"
                               name="installs"
                               value="{{ old('installs', $insight->installs) }}"
                               class="form-input block w-full sm:text-sm sm:leading-5"
                               type="text"/>
                    </div>

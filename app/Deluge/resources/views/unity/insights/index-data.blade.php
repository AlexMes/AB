                    ...
                    <tbody class="bg-white">
                        @foreach($insights as $insight)
                            <tr class="cursor-pointer hover:bg-gray-100"
                                onclick="window.location = '{{ route('deluge.unity.insights.show', $insight->id) }}'">
                                ...
                                <td class="px-5 py-5 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">
                                    <div class="font-semibold">{{ $insight->app->name }}</div>
                                    <div class="text-xs text-gray-600">{{ $insight->app_id }}</div>
                                </td>
                                <td class="px-5 py-5 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">
                                    <div class="font-semibold">{{ $insight->campaign->name }}</div>
                                ...
                                <td class="px-5 py-5 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">
                                    {{ $insight->created_at->format('M d, H:i:s') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                <nav class="flex items-center justify-between px-4 py-3 bg-white border-gray-200 sm:px-6">
                    ...
                    <div class="flex justify-between flex-1 sm:justify-end">
                        @if($insights->previousPageUrl())
                            <a href="{{ $insights->appends(request(['organization', 'app']))->previousPageUrl() }}"
                        ...
                        @if($insights->nextPageUrl())
                            <a href="{{ $insights->appends(request(['organization', 'app']))->nextPageUrl() }}"

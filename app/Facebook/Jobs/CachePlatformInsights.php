...
class CachePlatformInsights implements ShouldQueue
{
    use Dispatchable;
    use Queueable;
        ...
        return optional($this->retrieve())
            ...
            ->map(function ($insight) {
                return [
                    ...
                    'device_platform'    => $insight['device_platform'] ?? null,
    ...
    protected function update($insight)
    {
        PlatformInsights::query()
            ...
            ->where('device_platform', $insight['device_platform'])
            ->where('publisher_platform', $insight['publisher_platform'])
            ->where('impression_device', $insight['impression_device'])
            ->where('platform_position', $insight['platform_position'])
    ...
    {
        PlatformInsights::create($insight);
    }

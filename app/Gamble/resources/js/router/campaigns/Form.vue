    ...
    <header class="bg-white shadow-sm">
      <div class="flex items-center justify-between px-4 py-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <h1 class="text-lg font-semibold leading-6 text-gray-900">
          <span v-if="isEditing">Редактирование кампании</span>
          <span v-else>Создание кампании</span>
        </h1>
      </div>
    </header>
...
export default {
  ...
  computed: {
    ...
    cleanForm() {
      return {
        ...
        account_id: this.campaign.account !== null ? this.campaign.account.id : null,
        offer_id: this.campaign.offer !== null ? this.campaign.offer.id : null,
      };
    },
  },
  created() {
  ...
  methods: {
    ...
    cancel() {
      this.isEditing
        ? this.$router.push({name:'campaigns.show', params:{id: this.id}})
        : this.$router.push({name:'campaigns.index'});
    },
    create() {
      axios
        .post('/api/campaigns/', this.cleanForm)
        .then(r => {
          this.$router.push({
        ...
        .catch(err => {
          ...
          this.$toast.error({title: 'Unable to create campaign.', message: err.response.data.message});
        }).finally(()=> this.isBusy = false);
    },
    update() {
      axios
        .put(`/api/campaigns/${this.id}`, this.cleanForm)
        .then(r => {
          this.$router.push({
            name: 'campaigns.show',
            params: { id: r.data.id },

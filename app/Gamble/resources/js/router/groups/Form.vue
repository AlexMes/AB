    ...
    <header class="bg-white shadow-sm">
      <div class="flex items-center justify-between px-4 py-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <h1 class="text-lg font-semibold leading-6 text-gray-900">
          <span v-if="isEditing">Редактирование группы</span>
          <span v-else>Создание группы</span>
        </h1>
      </div>
    </header>
  ...
  methods: {
    load() {
      axios.get(`/api/groups/${this.id}`)
        ...
        .catch(err => this.$toast.error({title: 'Unable to load the group.', message: err.response.data.message}));
    },
    save() {
      this.isBusy = true;
    ...
    cancel() {
      this.isEditing
        ? this.$router.push({name:'groups.show', params:{id: this.id}})
        : this.$router.push({name:'groups.index'});
    },
    create() {
      axios
        .post('/api/groups/', this.cleanForm)
        .then(r => {
          this.$router.push({
            name: 'groups.index',
          });
        ...
        .catch(err => {
          ...
          this.$toast.error({title: 'Unable to create group.', message: err.response.data.message});
        }).finally(()=> this.isBusy = false);
    },
    update() {
      axios
        .put(`/api/groups/${this.id}`, this.cleanForm)
        .then(r => {
          this.$router.push({
            name: 'groups.show',
            params: { id: r.data.id },

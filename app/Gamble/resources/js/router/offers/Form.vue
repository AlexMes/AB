    ...
    <header class="bg-white shadow-sm">
      <div class="flex items-center justify-between px-4 py-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <h1 class="text-lg font-semibold leading-6 text-gray-900">
          <span v-if="isEditing">Редактирование офера</span>
          <span v-else>Создание офера</span>
        </h1>
      </div>
    </header>
  ...
  methods: {
    load() {
      axios.get(`/api/offers/${this.id}`)
        ...
        .catch(err => this.$toast.error({title: 'Unable to load the offer.', message: err.response.data.message}));
    },
    save() {
      this.isBusy = true;
    ...
    create() {
      axios
        .post('/api/offers/', this.cleanForm)
        .then(r => {
          this.$router.push({
            name: 'offers.index',
          });
        ...
        .catch(err => {
          ...
          this.$toast.error({title: 'Unable to create offer.', message: err.response.data.message});
        }).finally(()=> this.isBusy = false);
    },
    update() {
      axios
        .put(`/api/offers/${this.id}`, this.cleanForm)
        .then(r => {
          this.$router.push({
            name: 'offers.show',
            params: { id: r.data.id },

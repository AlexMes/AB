...
import TechCostListItem from '../../components/tech-costs/tech-costs-list-item';
export default {
  name: 'tech-costs-index',
  components: {TechCostListItem, Pagination},
  data: () => {
    return {
      techCosts: [],
      response: {},
  ...
  computed: {
    hasTechCosts() {
      return this.techCosts.length > 0;
    },
  },
  created() {
    this.load();
  ...
  methods: {
    load(page = 1) {
      axios.get('/api/tech-costs', {
        params: {
          page,
        },
      })
        .then(r => {
          this.techCosts = r.data.data;
          this.response = r.data;
        })
        .catch(err => this.$toast.error({title: 'Unable load tech costs.', message: err.response.data.message}));
    },
  },
};
</script>

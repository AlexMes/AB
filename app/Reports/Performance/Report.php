...
use App\Facebook\Ad;
use App\Queries\PerformanceDelugeInsights;
 ...
 * Class Report
 *
 * @package App\Reports\Performance
 */
class Report implements Responsable, Arrayable
{
    ...
    public function toArray()
    {
        ...
        $manualInsights = $this->manualInsights();
        ...
        return [
            ...
            'rows'    => Rows::build($insights, $manualInsights, $deposits, $this->level, $traffic, $hideFb)->toArray(),
            'summary' => Summary::build($insights, $manualInsights, $deposits, $this->level, $traffic, $hideFb)->toArray(),
            'period'  => [
     ...
     * @return \App\Insights[]|\Illuminate\Database\Eloquent\Builder[]|\Illuminate\Database\Eloquent\Collection
     */
    protected function manualInsights()
    {
        return PerformanceDelugeInsights::fetch()
            ->forPeriod($this->since, $this->until)
            ->forAccounts($this->accounts->pluck('account_id')->unique())
            ->forOffers($this->offers)
            ->forCampaign($this->utm_campaign)

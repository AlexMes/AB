...
class Report implements Responsable, Arrayable
{
    ...
    public const GROUP_DB_FIELDS = [
        self::GROUP_OFFICE      => 'office_id',
        self::GROUP_BRANCH      => 'branch_id',
        self::GROUP_USER        => 'user_id',
        self::GROUP_OFFER       => 'offer_id',
        self::GROUP_DESTINATION => 'destination_id',
        self::GROUP_GEO         => 'country_code',
        self::GROUP_MODEL       => 'office_id',
        self::GROUP_PRICE       => 'offer_id',
        self::GROUP_STATUS      => 'status',
        self::GROUP_APP         => 'app_id',
        self::GROUP_SOURCE      => 'traffic_source',
    ];
     ...
     *
     * @return \App\Reports\Revise2\Report
     */
    ...
    protected function prepareCrossFilters()
    {
        $groups = [];
        foreach (explode('-', $this->groupBy) as $group) {
            $groups[$group] = self::GROUP_DB_FIELDS[$group];
        }

        $leads = $this->leads()->get();
        foreach ($leads as $lead) {
            foreach ($groups as $group => $field) {
                $this->crossFilters[$group] = array_merge($this->crossFilters[$group] ?? [], [$lead->{$field}]);
            }
        }

        $deps = $this->deposits()->get();
        foreach ($deps as $dep) {
            foreach ($groups as $group => $field) {
                $this->crossFilters[$group] = array_merge($this->crossFilters[$group] ?? [], [$dep->{$field}]);
            }
        }
    }
        ...
        return DB::query()
            ...
            ->when($this->groupBy === self::GROUP_OFFICE, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([DB::raw('offices.name as group')])
                    ->from('offices')
                    ->whereIn('offices.id', $this->crossFilters[self::GROUP_OFFICE] ?? [])
            ...
            ->when($this->groupBy === self::GROUP_BRANCH, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([DB::raw('branches.name as group')])
                    ->from('branches')
                    ->whereIn('branches.id', $this->crossFilters[self::GROUP_BRANCH] ?? [])
            ...
            ->when($this->groupBy === self::GROUP_USER, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([DB::raw('users.name as group')])
                    ->from('users')
                    ->whereIn('users.id', $this->crossFilters[self::GROUP_USER] ?? [])
            ...
            ->when($this->groupBy === self::GROUP_OFFER, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([DB::raw('offers.name as group')])
                    ->from('offers')
                    ->whereIn('offers.id', $this->crossFilters[self::GROUP_OFFER] ?? [])
            ...
            ->when($this->groupBy === self::GROUP_OFFER_GEO, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([
                    ...
                    ->crossJoin('countries')
                    ->whereIn('offers.id', $this->crossFilters[self::GROUP_OFFER] ?? [])
                    ->whereIn('countries.code', $this->crossFilters[self::GROUP_GEO] ?? [])
                    ->leftJoinSub($this->deposits(), 'deps', function (JoinClause $join) {
            ...
            ->when($this->groupBy === self::GROUP_OFFICE_OFFER_DEST, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([
                    ...
                    ->whereIn('offices.id', $this->crossFilters[self::GROUP_OFFICE] ?? [])
                    ->whereIn('offers.id', $this->crossFilters[self::GROUP_OFFER] ?? [])
                    ->whereIn('lead_destinations.id', $this->crossFilters[self::GROUP_DESTINATION] ?? [])
                    ->leftJoinSub($this->deposits(), 'deps', function (JoinClause $join) {
                        return $join->on('offices.id', 'deps.office_id')
                            ->on('offers.id', 'deps.offer_id')
            ...
            ->when($this->groupBy === self::GROUP_OFFICE_OFFER_PRICE, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([
                    ...
                    ->whereIn('offices.id', $this->crossFilters[self::GROUP_OFFICE] ?? [])
                    ->whereIn('offers.id', $this->crossFilters[self::GROUP_OFFER] ?? [])
                    ->whereIn('lead_payment_conditions.office_id', $this->crossFilters[self::GROUP_MODEL] ?? [])
                    ->whereIn('lead_payment_conditions.offer_id', $this->crossFilters[self::GROUP_PRICE] ?? [])
                    ->leftJoinSub($this->deposits(), 'deps', function (JoinClause $join) {
                        return $join->on('offices.id', 'deps.office_id')
                            ->on('offers.id', 'deps.offer_id')
                ...
                return $builder->addSelect([
                    ...
                    ->from(
                        LeadOrderAssignment::selectRaw('distinct status')
                            ->whereIn('status', $this->crossFilters[self::GROUP_STATUS] ?? [])
                            ->whereNotNull('status'),
                        'assignments'
                    )
                    ->crossJoin('offices')
            ...
            ->when($this->groupBy === self::GROUP_OFFICE_OFFER_STATUS, function (\Illuminate\Database\Query\Builder $builder) {
                $statuses = Status::query()->whereIn('name', $this->crossFilters[self::GROUP_STATUS] ?? [])->pluck('name')
                    ->map(function ($item) {
                        return "('" . $item . "')";
                    });
            ...
            ->when($this->groupBy === self::GROUP_APP, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([DB::raw('apps.id as group')])
                    ->fromSub(
                        Lead::selectRaw('distinct app_id as id')
                            ->whereIn('app_id', $this->crossFilters[self::GROUP_APP] ?? [])
                            ->whereNotNull('app_id'),
                        'apps'
                    )
            ...
            ->when($this->groupBy === self::GROUP_OFFER_SOURCE, function (\Illuminate\Database\Query\Builder $builder) {
                return $builder->addSelect([
                    ...
                    ->fromSub(
                        Lead::selectRaw('distinct traffic_source as name')
                            ->whereIn('traffic_source', $this->crossFilters[self::GROUP_SOURCE] ?? [])
        ...
        return LeadOrderAssignment::visible()
            ...
            ->when($this->groupBy === self::GROUP_OFFICE_OFFER_GEO, function (Builder $builder) {
                return $builder->addSelect([
                    ...
                    ->leftJoin('ip_addresses', 'leads.ip', 'ip_addresses.ip')
                    ->groupBy(['leads_orders.office_id', 'lead_order_routes.offer_id', 'ip_addresses.country_code']);
            })
        ...
        return Deposit::visible()
            ...
            ->when($this->groupBy === self::GROUP_OFFICE_OFFER_GEO, function (Builder $builder) {
                return $builder->addSelect([
                    ...
                    ->join('leads', 'deposits.lead_id', 'leads.id')
                    ->leftJoin('ip_addresses', 'leads.ip', 'ip_addresses.ip')
                    ->groupBy(['deposits.office_id', 'deposits.offer_id', 'ip_addresses.country_code']);
            })
            ->when($this->groupBy === self::GROUP_USER_GEO, function (Builder $builder) {
                return $builder->addSelect([
                    'deposits.user_id',

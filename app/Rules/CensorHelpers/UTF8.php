...
namespace App\Rules\CensorHelpers;
...
class UTF8
{
    ...
    private static function _convert($data, $charset_from, $charset_to)
    {
        ...
        if (is_string($data)) {
            ...
            #charset_to
            if ($charset_to === 'cp1251' || $charset_to === 'cp1259') {
                return strtr($data, array_flip(self::$cp1259_table));
            }
        ...
        foreach (unpack($types[$type] . '*', $s) as $codepoint) {
            ...
            if ($hi !== false) {
                ...
                if ($lo < 0xDC00 || $lo > 0xDFFF) {
                    $a[] = "\xEF\xBF\xBD";
                } #U+FFFD REPLACEMENT CHARACTER (for broken char)
                else {
    ...
    public static function check($s, $is_strict = true)
    {
        ...
        for ($i = 0, $len = strlen($s); $i < $len; $i++) {
            ...
            if (($c & 0xE0) == 0xC0) {
                $n = 1;
            } #2 bytes 110bbbbb 10bbbbbb
            elseif (($c & 0xF0) == 0xE0) {
                $n = 2;
            } #3 bytes 1110bbbb 10bbbbbb 10bbbbbb
            elseif (($c & 0xF8) == 0xF0) {
                $n = 3;
            } #4 bytes 11110bbb 10bbbbbb 10bbbbbb 10bbbbbb
            elseif (($c & 0xFC) == 0xF8) {
                $n = 4;
            } #5 bytes 111110bb 10bbbbbb 10bbbbbb 10bbbbbb 10bbbbbb
            elseif (($c & 0xFE) == 0xFC) {
                $n = 5;
            } #6 bytes 1111110b 10bbbbbb 10bbbbbb 10bbbbbb 10bbbbbb 10bbbbbb
            else {
                return false;
    ...
    public static function blocks_check($data, $blocks)
    {
        ...
        if (is_string($data)) {
            ...
            $skip = []; #save to cache already checked symbols
    ...
    public static function to_unicode($s)
    {
        ...
        return array_map([__CLASS__, 'ord'], $a);
    }
    ...
    public static function from_unicode($a)
    {
        ...
        return implode('', array_map([__CLASS__, 'chr'], $a));
    }
    ...
    public static function strpos($s, $needle, $offset = null)
    {
        ...
        $byte_pos = $offset;
        do {
            if (($byte_pos = strpos($s, $needle, $byte_pos)) === false) {
                return false;
            }
    ...
    public static function unescape($data, $is_rawurlencode = false)
    {
        ...
        if (is_string($data)) {
            if (strpos($data, '%u') === false) {
                return $data;
            } #use strpos() for speed improving

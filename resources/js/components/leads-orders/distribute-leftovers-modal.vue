<template>
  <modal
    name="distribute-leftovers-modal"
    height="350"
    :adaptive="true"
    ...
    <div class="flex flex-col w-full p-6">
      ...
      <div class="flex w-full">
        <button
          ...
          :disabled="isBusy"
          @click="distributeLeftovers"
        >
          Выдать
        </button>
        <button
          class="button btn-secondary"
          @click="$modal.hide('distribute-leftovers-modal');"
        >
          Отмена
        </button>
      </div>
...
export default {
  ...
  methods: {
    beforeOpen(event) {
      if (!!event.params && !!event.params.offer) {
        this.filters.offers = [event.params.offer];
      }
    ...
    loadOffers() {
      axios.get('/api/offers', {params: {search: 'LO\\_'}})
        .then(({data}) => this.offers = data)
        .catch(err => this.$toast.error({title: 'Не удалось загрузить офферы.', message: err.response.data.message}));
    },
    distributeLeftovers() {
      this.isBusy = true;
      axios.post('/api/distribute-leftovers', this.cleanFilters)
        .then(() => {
          this.$modal.hide('distribute-leftovers-modal');
          this.$toast.success({title: 'OK', message: 'Выдача лидов запущена.'});
          this.errors = new ErrorBag();
        })
        .catch(err => {
          this.errors.fromResponse(err);
          this.$toast.error({title: 'Не удалось запустить выдачу лидов.', message: err.response.data.message});
        })

    ...
    <div class="flex flex-col container mx-auto">
      <div class="flex flex-col md:flex-row w-full justify-between items-start md:items-center">
        <h1 class="text-gray-700 flex">
          Office performance Affs report
        </h1>
        <span>
          <span
            v-if="report.period"
      ...
      <div class="flex flex-col my-6">
        <div class="flex flex-wrap md:flex-no-wrap justify-between items-start md:items-center">
          ...
          <div class="flex flex-col align-middle mx-2 my-2 md:my-0 w-full">
            <label class="mb-2">Аффилиаты</label>
            <mutiselect
              v-model="filters.affiliates"
              :show-labels="false"
              :multiple="true"
              :options="affiliates"
              placeholder="Выберите аффилиатов"
              track-by="id"
              label="name"
            ></mutiselect>
          </div>
...
export default {
  name: 'office-affiliate-performance',
  components: {
    DatePicker,
  },
  data: () => ({
  ...
  computed: {
    ...
    cleanFilters() {
      return {
        ...
        affiliates: this.filters.affiliates === null ? null : this.filters.affiliates.map(affiliate => affiliate.id),
  ...
  methods: {
    load() {
      this.isBusy = true;
      axios
        .get('/api/reports/office-affiliate-performance', { params: this.cleanFilters })

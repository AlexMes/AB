...
<script>
import OfficeGroupOfficeListItem from '../../../components/settings/office-group-office-list-item';
export default {
  name: 'office-groups-offices',
  components: {OfficeGroupOfficeListItem},
  props: {
    id: {
      type: [Number, String],
      required: true,
  ...
  methods: {
    load(page = 1) {
      axios
        .get(`/api/office-groups/${this.id}/offices`, { params: { page } })
        .then(({ data }) => {
    ...
    add() {
      if (this.office !== null) {
        axios
          .post(`/api/office-groups/${this.id}/offices`, {office_id: this.office.id})
          .then(response => {
    ...
    remove(office) {
      axios.delete(`/api/office-groups/${this.id}/offices/${office.id}`)
        .then(response => {
